...
fn body_encoding(i: &[u8]) -> IResult<&[u8], ContentEncoding<'_>> {
    alt((
        // RFC 2045 defaults an omitted Content-Transfer-Encoding to 7bit.
        // Maddy represents that omission as NIL in BODYSTRUCTURE responses.
...
fn body_param(i: &[u8]) -> IResult<&[u8], BodyParams<'_>> {
    alt((
        map(nil, |_| None),
        // RFC 3501 requires NIL or at least one key/value pair here, but
        // Maddy 0.8 emits an empty list for MIME parts without Content-Type
        // parameters. Treat that widely recoverable server deviation as NIL
        // instead of rejecting the message's entire FETCH response.
        map(tag("()"), |_| None),
        map(
...
fn body_type_multipart(i: &[u8]) -> IResult<&[u8], BodyStructure<'_>> {
    map(
        // RFC 3501 writes multipart body parts back-to-back. Maddy 0.8
        // separates them with whitespace, so accept either representation.
        tuple((
            many1(preceded(space0, body)),
            tag(" "),
            string_utf8,
            body_ext_mpart,
        )),
...
mod tests {
    ...
    fn test_body_param_data() {
        ...
        assert_matches!(body_param(br#"()"#), Ok((EMPTY, None)));
    ...
    #[test]
    fn tolerates_maddy_empty_params_in_single_text_part() {
        let input = br#"("text" "plain" () NIL NIL "7bit" 36 1 NIL ("attachment" ("filename" "itest-note.txt")) NIL NIL)"#;
        assert_matches!(body(input), Ok((EMPTY, BodyStructure::Text { .. })));
    }

    #[test]
    fn tolerates_maddy_empty_params_in_multipart_body() {
        let input = br#"(("text" "plain" ("charset" "utf-8") NIL NIL "7bit" 35 1 NIL NIL NIL NIL) ("text" "plain" () NIL NIL "7bit" 36 1 NIL ("attachment" ("filename" "itest-note.txt")) NIL NIL) "mixed" ("boundary" "fixture-boundary") NIL NIL NIL)"#;
        assert_matches!(body(input), Ok((EMPTY, BodyStructure::Multipart { .. })));
    }

    #[test]
    fn tolerates_maddy_empty_params_and_missing_transfer_encoding() {
        let response = concat!(
            r#"* 6 FETCH (UID 7 BODYSTRUCTURE (("text" "plain" ("charset" "utf-8") NIL NIL "7bit" 35 1 NIL NIL NIL NIL) ("text" "plain" () NIL NIL NIL 36 1 NIL ("attachment" ("filename" "itest-note.txt")) NIL NIL) "mixed" ("boundary" "fixture-boundary") NIL NIL NIL))"#,
            "\r\n"
        )
        .as_bytes();

        assert_matches!(
            crate::parser::parse_response(response),
            Ok((EMPTY, Response::Fetch(_, attrs))) => {
                assert!(attrs.iter().any(|attr| matches!(attr, AttributeValue::BodyStructure(_))));
            }
        );
    }
